...
use crate::ui::glow::ui::{Container, HAttach, ImageBuilder, TextBuilder, VAttach};

use super::slot_mapping::SlotMapping;
use super::{Inventory, InventoryType, Item, Material};
...
impl Inventory for EnchantmentTableInventory {
    ...
    fn init(
        ...
        let ench_button = ImageBuilder::new()
            ...
            basic_elements
                ...
                .add_click_func(move |this, game| {
                    if this.texture_coords == BUTTON_FOCUSED {
                        game.server.load().as_ref().unwrap().clone().write_packet(
        ...
        let level_requ = ImageBuilder::new()
            .alignment(VAttach::Top, HAttach::Left)
        ...
        let button_text = TextBuilder::new()
            .alignment(VAttach::Top, HAttach::Left)
            .scale_x(icon_scale / 2.0)
            .scale_y(icon_scale / 2.0)
            .text("");
