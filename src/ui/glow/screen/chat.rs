...
use crate::ui::glow::render::hud::{self, Hud, START_TICKS};
...
use super::{Screen, ScreenSystem, ScreenType};
...
impl super::Screen for Chat {
    ...
    fn on_key_press(&mut self, key: (Key, PhysicalKey), down: bool, game: &Arc<Game>) {
        ...
        if key.0.eq_ignore_case('v') && ctx.input_ctx.read().is_ctrl_pressed {
            if let Ok(clipboard) = ctx.clipboard_provider.lock().get_contents() {
        ...
        if let Some(str) = key.0.to_text() {
            ...
            if !ILLEGAL_CHARS.iter().any(|illegal| curr == *illegal) {
                if self.written.len()
                    >= if game.server.load().as_ref().unwrap().mapped_protocol_version >= Version::V1_11 {
                        MAX_MESSAGE_LENGTH_SINCE_1_11
                    } else {
                        MAX_MESSAGE_LENGTH_PRE_1_11
                    }
