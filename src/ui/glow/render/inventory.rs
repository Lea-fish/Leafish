...
use crate::ui::glow::inventory::{Inventory, InventoryContext, Item};
use crate::ui::glow::screen::{Screen, ScreenSystem};
use crate::ui::glow::ui::{Container, HAttach, ImageBuilder, ImageRef, TextBoxRef, TextBuilder, TextRef, VAttach};
use crate::Game;
...
impl Screen for InventoryWindow {
    ...
    fn on_key_press(&mut self, key: (Key, PhysicalKey), down: bool, _game: &Arc<Game>) {
        if key.0 == Key::Named(NamedKey::Escape) && !down {
            self.inventory_context
                .write()
                .try_close_inventory(&ctx().screen_sys);
        }
    }
...
impl InventoryWindow {
    pub fn draw_item(
        ...
        let image = ImageBuilder::new()
            ...
            .position(x, y)
            .alignment(v_attach, HAttach::Left)
