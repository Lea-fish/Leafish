...
impl UIState {
    ...
    pub fn tick(&mut self, width: u32, height: u32) {
        ...
        if self.count > 0 {
            ...
            if self.max_index < self.count {
                let (data, ty) = super::generate_element_buffer(self.count);
                self.index_type = ty;
    ...
    pub fn character_texture(&mut self, c: char) -> Texture {
        let raw = c as u32;
        let page = raw >> 8;
        ...
        if self.font_pages[page as usize].is_none() {
            ...
            self.font_pages[page as usize] = Some(Renderer::get_texture(&textures, &name));
        }
