...
use crate::ui::glow::inventory::slot_mapping::SlotMapping;
use crate::ui::glow::inventory::Item;
use crate::ui::glow::screen::{Screen, ScreenSystem, ScreenType, SettingsMenu};
use crate::ui::glow::ui::{Container, FormattedBuilder, FormattedRef, HAttach, ImageBuilder, ImageRef, TextBuilder, TextRef, VAttach};
use crate::{format, Game};
...
impl Screen for Hud {
    ...
    fn on_key_press(&mut self, key: (Key, PhysicalKey), down: bool, game: &Arc<Game>) {
        let ctx = ctx();
        let focused = game.focused.load(AtomicOrdering::Acquire);
        if key.0 == Key::Named(NamedKey::Escape) && !down && focused {
            ctx.screen_sys
                .add_screen(Box::new(SettingsMenu::new(game.settings.clone(), true)));
            return;
        }
        match key.1 {
...
impl Hud {
    ...
    pub fn draw_item(
        ...
        let item_image = ImageBuilder::new()
            .draw_index(HUD_PRIORITY)
