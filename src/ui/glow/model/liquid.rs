...
use crate::ui::glow::render::{Renderer, TextureManager};
...
pub fn render_liquid<W: Write>(
    ...
    let tex = match snapshot.get_block(x, y, z) {
        block::Block::Water { .. } => {
            Renderer::get_texture(&textures, "minecraft:block/water_still")
        }
        block::Block::Lava { .. } => {
            Renderer::get_texture(&textures, "minecraft:block/lava_still")
        }
