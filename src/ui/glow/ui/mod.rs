...
macro_rules! define_elements {
    (
        ...
        impl Element {
            ...
            fn tick(&self, renderer: &Arc<Renderer>) {
                match *self {
                    $(
            ...
            fn draw(&self, renderer: &Arc<Renderer>, r: &Region, sw: f64, sh: f64, width: f64, height: f64, delta: f64) -> RefMut<[u8]> {
                match *self {
                    $(
                        Element::$name(ref inner) => {
            ...
            fn hover_at(&self, r: &Region, game: &Arc<crate::Game>, mx: f64, my: f64, sw: f64, sh: f64) -> bool {
                match *self {
                    $(
                        Element::$name(ref inner) => {
                            let mut el = inner.borrow_mut();
            ...
            fn click_at(&self, r: &Region, game: &Arc<crate::Game>, mx: f64, my: f64, sw: f64, sh: f64) -> bool {
                match *self {
                    $(
                        Element::$name(ref inner) => {
                            let mut el = inner.borrow_mut();
            ...
            fn key_press(&self, game: &Arc<crate::Game>, key: Key, down: bool, ctrl_pressed: bool) {
                match *self {
                    $(
                        Element::$name(ref inner) => {
                            let mut el = inner.borrow_mut();
            ...
            fn key_type(&self, game: &Arc<crate::Game>, c: char) {
                match *self {
                    $(
                        Element::$name(ref inner) => {
                            let mut el = inner.borrow_mut();
...
impl Container {
    ...
    pub fn tick(&mut self, renderer: &Arc<Renderer>, delta: f64, width: f64, height: f64) {
    ...
    pub fn hover_at(&mut self, game: &Arc<crate::Game>, x: f64, y: f64, width: f64, height: f64) {
        let (sw, sh) = match self.mode {
            Mode::Scaled => (SCALED_WIDTH / width, SCALED_HEIGHT / height),
            Mode::Unscaled(scale) => (scale, scale),
        };
    ...
    pub fn click_at(&mut self, game: &Arc<crate::Game>, x: f64, y: f64, width: f64, height: f64) {
        let (sw, sh) = match self.mode {
            Mode::Scaled => (SCALED_WIDTH / width, SCALED_HEIGHT / height),
            Mode::Unscaled(scale) => (scale, scale),
        };
    ...
    pub fn key_press(&mut self, game: &Arc<crate::Game>, key: Key, down: bool, ctrl_pressed: bool) {
...
trait UIElement {
    ...
    fn key_press(&mut self, _game: &Arc<crate::Game>, _key: Key, _down: bool, _ctrl_pressed: bool) {}
    fn key_type(&mut self, _game: &Arc<crate::Game>, _c: char) {}
    fn tick(&mut self, renderer: &Arc<Renderer>);
}
...
macro_rules! element {
    (
        ...
        pub struct $name {
            ...
            hover_funcs: Vec<Box<dyn Fn(&mut $name, bool, &Arc<crate::Game>) -> bool>>,
            hover_state: bool,
            click_funcs: Vec<Box<dyn Fn(&mut $name, &Arc<crate::Game>) -> bool>>,
        ...
        impl $name {
            ...
            fn super_draw(&mut self, renderer: &Arc<Renderer>, super_region: &Region, sw: f64, sh: f64, width: f64, height: f64, delta: f64) {
                if !self.needs_rebuild {
                    let (w, h) = self.get_size();
            ...
            fn super_tick(&mut self, renderer: &Arc<Renderer>) {
                for &(_, ref e) in &self.elements {
                    e.tick(renderer);
                }
            }

            fn hover_at(&mut self, super_region: &Region, game: &Arc<crate::Game>, mx: f64, my: f64, sw: f64, sh: f64) -> bool {
                use std::mem;
            ...
            pub fn add_hover_func<F: Fn(&mut $name, bool, &Arc<crate::Game>) -> bool + 'static>(&mut self, func: F) {
                self.hover_funcs.push(Box::new(func));
            }

            fn click_at(&mut self, super_region: &Region, game: &Arc<crate::Game>, mx: f64, my: f64, sw: f64, sh: f64) -> bool {
                use std::mem;
                let mut handle_self = true;
                for e in &self.elements {
                    let r = Container::compute_draw_region(&e.1, sw, sh, &super_region);
            ...
            pub fn add_click_func<F: Fn(&mut $name, &Arc<crate::Game>) -> bool + 'static>(&mut self, func: F) {
...
impl UIElement for Image {
    fn draw(
        ...
        if self.check_rebuild() {
            self.data.clear();
            let texture = Renderer::get_texture(renderer.get_textures_ref(), &self.texture);
            let mut element = super::render::ui::UIElement::new(
                &texture,
                r.x,
                r.y,
                r.w,
...
impl UIElement for Button {
    fn draw(
        ...
        if self.check_rebuild() {
            ...
            let texture = Renderer::get_texture(renderer.get_textures_ref(), "gui/widgets")
                .relative(0.0, offset / 256.0, 200.0 / 256.0, 20.0 / 256.0);

            self.data.extend(
                super::render::ui::UIElement::new(
                    &texture,
                    r.x,
                    r.y,
                    4.0 * sw,
...
type TextBoxSubmitCallback = dyn Fn(&mut TextBox, &Arc<crate::Game>);
...
impl UIElement for TextBox {
    ...
    fn key_press(&mut self, game: &Arc<crate::Game>, key: Key, down: bool, ctrl_pressed: bool) {
        if key == Key::Named(NamedKey::Enter) && !down {
            ...
            let ctx = ctx();
            if let Ok(text) = ctx.clipboard_provider.lock().get_contents() {
                self.input.push_str(&text)
            }
        }
    }

    fn key_type(&mut self, _game: &Arc<crate::Game>, c: char) {
        if c == '\x7f' || c == '\x08' {
            // Backspace
            self.input.pop();
            return;
...
impl TextBox {
    pub fn add_submit_func<F: Fn(&mut TextBox, &Arc<crate::Game>) + 'static>(&mut self, f: F) {
