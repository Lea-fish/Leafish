use super::{resolve_textures, Bounds, EntityType, GameInfo, Light, Position, Rotation, Velocity};
use crate::ui::glow::render::model::{self, DEFAULT};
use crate::ui::glow::render::Renderer;
use crate::{ecs::Manager, ui::glow::render::model::ModelHandle};
...
pub fn added_slime(renderer: Res<RendererResource>, mut query: Query<&mut SlimeModel>) {
    ...
    for mut slime_model in query.iter_mut() {
        ...
        let model = renderer.models.lock().create_model(
            DEFAULT,
            vec![
