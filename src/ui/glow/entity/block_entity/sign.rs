...
use crate::ui::glow::render::model::{self, FormatState, ModelHandle, DEFAULT};
...
fn add_sign(
    ...
    let tex = Renderer::get_texture(renderer.get_textures_ref(), &path);
    ...
    let mut models = renderer.models.lock();
    let model = models.create_model(DEFAULT, vec![verts], renderer.clone());
