use super::slime::{added_slime, update_slime};
use super::zombie::{added_zombie, update_zombie};
use super::{
    Bounds, Digging, EntityType, GameInfo, Gravity, Light, MouseButtons, Position, Rotation, TargetPosition, TargetRotation, Velocity
};
...
use crate::ui::glow::entity::resolve_textures;
use crate::ui::glow::render::model::{self, FormatState, ModelHandle};
...
fn add_player(renderer: Arc<Renderer>, player_model: &mut PlayerModel) {
    ...
    let skin = if let Some(url) = player_model.skin_url.load().as_ref() {
        ...
        Renderer::get_texture(renderer.get_textures_ref(), "entity/steve")
    };
