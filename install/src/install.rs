...
pub fn setup_launcher_wrapper(prefix: &str) -> anyhow::Result<bool> {
    ...
    println!("dir: {}", format!("{}{}", prefix, DIR_PATH));

    // create directory if necessary
    fs::create_dir(format!("{}{}", prefix, DIR_PATH))?;
    ...
    json.write_all(serde_json::to_string_pretty(&Description {
        ...
        minecraft_arguments: "--username ${auth_player_name} --version ${version_name} --gameDir ${game_directory} --assetsDir ${assets_root} --assetIndex ${assets_index_name} --uuid ${auth_uuid} --accessToken ${auth_access_token} --userProperties ${user_properties} --userType ${user_type}".to_string(),
    })?.as_bytes())?;
